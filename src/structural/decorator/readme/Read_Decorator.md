...
- 객체들을 사용하는 코드를 훼손하지 않으면서 런타임에 추가 행동들을 객체들에 할당할 수 있어야 할 때 사용
  - 비즈니스 로직을 계층으로 구성하여 각 계층에 데코레이터를 생성하고 런타잉에 이 로직의 다양한 조합들로 객체들을 구성할 수 있도록 함
  - 이러한 모든 객체가 공통 인터페이스를 따르기 때문에 클라이언트 코드는 해당 모든 객체를 같은 방식으로 다룰 수 있음
- 상속을 사용하여 객체의 행동을 확장하는 것이 어색하거나 불가능할 때 사용
  - 클래스의 추가 확장을 방지하는 final 클래스인 경우, 기존 행동들을 재사용할 수 있는 데코레이터 패턴을 사용하여 클래스를 자체 래퍼로 래핑하는 것
